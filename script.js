...
// --- Paper Airplane Scroll Animation ---
(function () {
  ...
  // S-curve path across 6 pages of content
  function getPosition(t) {
    ...
    const amplitude = (rightEdge - leftEdge) / 2;

    // Fast blend from start.x to targetX (swoops out in the first 5% of scroll)
  ...
  function getAngle(t) {
    const delta = 0.003;
    const a = getPosition(Math.max(0, t - delta));
    const b = getPosition(Math.min(1, t + delta));
    return Math.atan2(b.y - a.y, b.x - a.x);
  }

  // Park airplane exactly over the logo icon
  function parkAirplane() {
    const pos = getLogoPos();
    airplane.style.transform = `translate(${pos.x - 12}px, ${pos.y - 12}px) rotate(0rad)`;
    airplane.style.opacity = '0'; // Hidden — the static logo icon is visible
    logoIcon.style.opacity = '1';
    currentAngle = 0; // Reset angle when parked to match logo rotation
  }
  ...
  function updateAirplane() {
    ...
    targetT = docHeight > 0 ? Math.min(scrollTop / docHeight, 1) : 0;
  ...
  window.addEventListener('scroll', updateAirplane, { passive: true });
